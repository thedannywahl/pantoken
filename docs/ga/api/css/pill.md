...
| `.-render-icon-<name>` | <span class="instui-pill -color-danger pantoken-doc-tag">As dáta</span> — The former `renderIcon` prop; still works as an alias, but use `-icon-<name>` instead. |
...
- [badge](/ga/api/css/badge.md) — A badge is the count or notification counterpart.
- [tag](/ga/api/css/tag.md) — A tag is the removable, form-oriented counterpart.
