...
`:where(body)` — Տպագրական լռելյալներ հում HTML-ի համար — վերնագրեր, պարբերքեր, ցանկեր, հղումներ և կոդ — կիրառված ինքնաբերաբար, որտեղ այն ներմուծված է (լռելյալ `:where(body)`); փոխանցել `options.scope`-ը նպատակ դարձնել տարբեր բովանդակության արմատ փոխարենը (օր. `.vp-doc`):

**Աղբյուր:** [index.ts](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/rules/prose/index.ts)
