...
> [!TIP]
> **JS Requirement** — Այս բաղադրիչը չի ունի իր սեփական CSS-ը — դրա նշումը և վարքը ամբողջապես գալիս են `@pantoken/interactions`-ից: Դրա `--max-lines-auto` փոփոխիչը վարվում է այդ վարքի կողմից: Տես ստորև գտնվող [փոփոխիչների աղյուսակը](#modifiers):
