...
<!-- js-requirement -->
> [!TIP]
> **JS բարելավում** — Այս բաղադրիչի CSS-ը ցուցադրվում և աշխատում է ինքնուրույն; զուգակցեք այն `@pantoken/interactions`-ի հետ ինտերակտիվ վարքագիծ ավելացնելու համար: Դրա `-should-animate` փոփոխիչը վարվում է այդ վարքագծով: Տե՛ս ստորև գտնվող [փոփոխիչ աղյուսակ](#modifiers):
