...
`.instui-calendar` — Ստատիկ ամսային ցանց նավիգացիայով, շաբաթվա օրերի վերնագրերով և օր բջիջներով:

Տե՛ս `calendar.day` անդամը անհատական օր բջիջների համար:

**Աղբյուր.** [calendar.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/calendar/calendar.css)

## Մուտքականություն

Բացահայտել ցանցը `role="table"` և նկարագրական `aria-label` հետ, և տալ յուրաքանչյուր նավիգացիոն կոճակին իր սեփական `aria-label`:
...
| Մաս | Նկարագիր |
| --- | --- |
| `.grid` | Յոթ սյունակային օր ցանց: |
| `.nav` | Ամսային նավիգացիայի տող: |
| `.weekday` | Շաբաթվա օրի սյունակի վերնագիր: |
...
- [calendar.day](/hy/api/css/calendar.day.md)
