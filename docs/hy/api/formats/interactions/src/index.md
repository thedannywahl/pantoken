[pantoken](../../../index.md) / interactions

# interactions

`@pantoken/interactions` - մեկ բաղադրիչի ինտերակցիայի փաթեթներ InstUI բաղադրիչների համար:

## Interfaces

- [ProgressCircleOptions](interfaces/ProgressCircleOptions.md)
- [ProgressCircleHandle](interfaces/ProgressCircleHandle.md)
- [RemoveOptions](interfaces/RemoveOptions.md)
- [RemoveHandle](interfaces/RemoveHandle.md)
- [TooltipOptions](interfaces/TooltipOptions.md)
- [TooltipHandle](interfaces/TooltipHandle.md)
- [TruncateAutoOptions](interfaces/TruncateAutoOptions.md)
- [TruncateAutoHandle](interfaces/TruncateAutoHandle.md)
- [CommandEventish](interfaces/CommandEventish.md)

## Type Aliases

- [CommandHandler](type-aliases/CommandHandler.md)
- [OnCommand](type-aliases/OnCommand.md)

## Variables

- [SPACING\_ATTRS](variables/SPACING_ATTRS.md)

## Functions

- [initCloseButton](functions/initCloseButton.md)
- [initInPlaceEdit](functions/initInPlaceEdit.md)
- [initModal](functions/initModal.md)
- [initProgressCircle](functions/initProgressCircle.md)
- [initRemove](functions/initRemove.md)
- [initResponsiveOverlay](functions/initResponsiveOverlay.md)
- [initTooltip](functions/initTooltip.md)
- [syncTruncateAutoLines](functions/syncTruncateAutoLines.md)
- [initTruncateAuto](functions/initTruncateAuto.md)
- [makeOnCommand](functions/makeOnCommand.md)
- [syncInvoker](functions/syncInvoker.md)
- [resolveSpace](functions/resolveSpace.md)
- [spacingValue](functions/spacingValue.md)
- [applySpacing](functions/applySpacing.md)
