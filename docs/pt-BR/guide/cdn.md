...
pantoken publica cada pacote no npm, então você pode puxar tokens, componentes e web components direto
de um CDN — sem etapa de build, sem bundler. Esta página cobre a URL combinada de CSS (com um
construtor interativo), além dos drop-ins de web-component.

## A base de tokens

Todo componente pantoken lê `--instui-*` custom properties de uma folha de tokens na página. Duas
variantes são distribuídas:

- `@pantoken/css/dist/style.lean.css` — a fundação recomendada no CDN. Carrega todos os tokens exceto o
  conjunto completo de ícones, então tem cerca de 23 KB gzipped.
- `@pantoken/css/dist/style.css` — a folha completa, incluindo aproximadamente 1.777 tokens de glifo de ícone
  (`--instui-icon-*`). Cerca de 140 KB gzipped. Carregue esta se você referenciar ícones amplamente via
  `var(--instui-icon-*)`.

A escala de elevação e as variáveis do focus-ring estão em ambas as folhas, então sombras e o anel de foco funcionam com
apenas a fundação carregada.

## Escolha seus componentes e ícones

O [seletor CDN interativo](/guide/cdn-picker) constrói URLs combinadas jsDelivr para CSS e trechos para pacotes JavaScript. Abra-o, marque o que precisa e copie a saída gerada.
...
0.5 KB gzipped — os 11 ícones que o conjunto de componentes usa) sempre que você escolhe a folha enxuta. A folha completa
já os contém.
...
eles são utilitários de override, então só realmente sobreescrevem a regra de um componente quando chegam
depois dela na cascata. A URL combinada acima já os ordena para você. Fontes são a única exceção:
...
Marque **All components** no seletor para trocá-lo para o barrel, ou aponte para ele você mesmo (cerca de 141 KB
gzipped) junto com a folha de tokens:
...
Para um drop-in sem módulos, carregue a build IIFE. Ela agrupa suas dependências e auto-registra todo
elemento ao carregar, expondo uma global `PantokenWebComponents`:
...
É maior que o caminho ESM — embute `@pantoken/components` e `@pantoken/icons` — então recorra a ele
apenas quando não for possível usar módulos.

## Fixando versões

As URLs acima — e as que o seletor escreve — acompanham a release mais recente. Faça pin de uma major (ou exata)
versão para produção — por exemplo `@pantoken/css@0` — para que uma atualização não te surpreenda.
