...
<!-- js-requirement -->
> [!TIP]
> **Aprimoramento JS** — This component's CSS renders and works on its own; pair it with `@pantoken/interactions` to add the interactive behavior. Its `-timeout` modifier is driven by that behavior. See the [modifier table below](#modifiers).
...
- [close-button](/pt-BR/api/css/close-button.md) — The dismiss control an alert may include.
