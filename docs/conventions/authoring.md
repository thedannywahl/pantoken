...
- Only the `css` body stays a template literal. **Tag it ` css` ``and put a`// prettier-ignore`
  line right before it** (before the template for a direct arrow body, before `return` for a block).
  oxfmt treats ` css` `/` styled` ` as embedded CSS and corrupts interpolated selectors
  (`${p}foo.-mod` → `${p}foo .-mod`, a different meaning) otherwise. `embeddedLanguageFormatting:
"off"` does **not** stop it. See the engineering log.
