...
<!-- js-requirement -->
> [!TIP]
> **Ενίσχυση JS** — This component's CSS renders and works on its own; pair it with `@pantoken/interactions` to add the interactive behavior. Its `-timeout` modifier is driven by that behavior. See the [modifier table below](#modifiers).
...
| `.-render-custom-icon-*` | <span class="instui-pill -color-danger pantoken-doc-tag">Καταργημένο</span> — The former `renderCustomIcon` prop; still works as an alias, but use `-icon-<name>` (or override `--pantoken-alert-glyph`) instead. |
...
| `.-variant-error` | <span class="instui-pill -color-danger pantoken-doc-tag">Καταργημένο</span> — use `.-color-danger`. |
| `.-variant-info` | <span class="instui-pill -color-danger pantoken-doc-tag">Καταργημένο</span> — use `.-color-info`. |
| `.-variant-success` | <span class="instui-pill -color-danger pantoken-doc-tag">Καταργημένο</span> — use `.-color-success`. |
| `.-variant-warning` | <span class="instui-pill -color-danger pantoken-doc-tag">Καταργημένο</span> — use `.-color-warning`. |
| `.-without-shadow` | Remove the default elevation shadow (InstUI `hasShadow={false}`). |

## Ψευδοστοιχεία

| Ψευδοστοιχείο | Περιγραφή |
| --- | --- |
...
- [close-button](/el/api/css/close-button.md)

## Σχετιζόμενα

- [close-button](/el/api/css/close-button.md) — The dismiss control an alert may include.
