# L'interface en ligne de commande pantoken

`@pantoken/cli` fournit `pantoken generate <target>`, qui écrit la source des jetons dans un dépôt cible.
S'en servir lorsqu'une plateforme a besoin de code généré plutôt que d'une dépendance d'exécution — applications natives,
...
| `wordpress` | Un `theme.json` pour block-theme.                                                          |
...
| `icon-font` | Une police d'icônes web (TTF, WOFF2), son CSS, et une carte des points de code.            |
| `pendo`     | Le `global.css` stylisé Instructure pour les guides Pendo.                                 |
...
- `--class <Name>` — le nom de type ou de package généré, pour les cibles qui en ont besoin.
...
Plusieurs cibles exposent aussi une fonction simple, afin que vous puissiez les appeler depuis votre propre processus de build plutôt que
d'utiliser l'outil CLI. Voir la [référence API](/api/) pour chaque package de plateforme.
