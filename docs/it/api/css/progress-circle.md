...
<!-- js-requirement -->
> [!TIP]
> **Miglioria JS** — This component's CSS renders and works on its own; pair it with `@pantoken/interactions` to add the interactive behavior. Its `-should-animate` modifier is driven by that behavior. See the [modifier table below](#modifiers).
...
- [progress](/it/api/css/progress.md) — The linear bar form of the same determinate progress.
