...
<!-- js-requirement -->
> [!TIP]
> **JS 拡張** — This component's CSS renders and works on its own; pair it with `@pantoken/interactions` to add the interactive behavior. Its `-timeout` modifier is driven by that behavior. See the [modifier table below](#modifiers).
...
| `.-render-custom-icon-*` | <span class="instui-pill -color-danger pantoken-doc-tag">非推奨</span> — The former `renderCustomIcon` prop; still works as an alias, but use `-icon-<name>` (or override `--pantoken-alert-glyph`) instead. |
...
| `.-variant-error` | <span class="instui-pill -color-danger pantoken-doc-tag">非推奨</span> — use `.-color-danger`. |
| `.-variant-info` | <span class="instui-pill -color-danger pantoken-doc-tag">非推奨</span> — use `.-color-info`. |
| `.-variant-success` | <span class="instui-pill -color-danger pantoken-doc-tag">非推奨</span> — use `.-color-success`. |
| `.-variant-warning` | <span class="instui-pill -color-danger pantoken-doc-tag">非推奨</span> — use `.-color-warning`. |
...
- [close-button](/ja/api/css/close-button.md)

## 関連項目

- [close-button](/ja/api/css/close-button.md) — The dismiss control an alert may include.
