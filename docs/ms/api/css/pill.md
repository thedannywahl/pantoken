...
| `.-render-icon-<name>` | <span class="instui-pill -color-danger pantoken-doc-tag">Usang</span> — The former `renderIcon` prop; still works as an alias, but use `-icon-<name>` instead. |
...
- [badge](/ms/api/css/badge.md) — A badge is the count or notification counterpart.
- [tag](/ms/api/css/tag.md) — A tag is the removable, form-oriented counterpart.
