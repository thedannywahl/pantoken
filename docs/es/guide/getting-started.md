...
Pantoken toma los tokens de diseño e iconos de [Instructure UI](https://instructure.design), los resuelve una vez y remodela ese único
modelo en paquetes para muchas plataformas: hojas de estilo simples, SCSS y Less, React y Vue y Svelte,
Tailwind y Panda, nativo Swift y Kotlin, WordPress y Drupal, Figma y más.

Instala el paquete más pequeño que se ajuste a tu tarea. Todo también se reexporta mediante el paquete unificado
`pantoken`, por lo que puedes empezar allí y afinar después.

## Crear un proyecto inicial

La forma más rápida de probar pantoken: generar un proyecto inicial con él ya instalado y configurado.

```sh
npx create-pantoken-app
```

Plataformas: `components` (HTML/CSS simple), `react`, `vue`, `svelte`, `web-components`, `angular`. Consulta
[`@pantoken/scaffold`](https://www.npmjs.com/package/@pantoken/scaffold) para `--dir <path>` y
uso programático.

¿Usando un agente de codificación AI? No se necesita instalación — apúntalo directamente a la skill:
...
Instala la hoja de estilo e impórtala una vez. Define cada propiedad `--instui-*`, por lo que las referencias
se hacen directamente desde tu propio CSS.
...
icono como `mask-image` o `background-image` — no se necesita importación por icono.
...
`@pantoken/icons` expone dos exportaciones con nombre. Usa `iconsByName` para obtener un icono sin iterar
sobre todo el array:
...
nivel. Para una carga ligera solo con CSS, usa el [CDN picker](/guide/cdn-picker) para generar una URL combinada
solo con los iconos que necesitas.

## Generar para una plataforma nativa

El CLI escribe la fuente de tokens en un repositorio destino. No se necesita instalación más allá del runner:

```sh
npx @pantoken/cli generate swift --out ./ios/Tokens --icons arrow-left,check-mark
```

Consulta [el CLI de pantoken](/guide/cli) para cada destino.

## Pistas para autoría en VS Code
...
1. Señala VS Code al JSON de datos personalizados incluido desde tu espacio de trabajo consumidor:
...
- [Arquitectura](/guide/architecture) — cómo encajan el modelo de tokens, el núcleo y las salidas.
- [Referencia API](/api/) — cada símbolo exportado, generado desde la fuente.
