...
| `.-render-icon-<name>` | <span class="instui-pill -color-danger pantoken-doc-tag">Deprekereda</span> — The former `renderIcon` prop; still works as an alias, but use `-icon-<name>` instead. |
...
- [badge](/se/api/css/badge.md) — A badge is the count or notification counterpart.
- [tag](/se/api/css/tag.md) — A tag is the removable, form-oriented counterpart.
