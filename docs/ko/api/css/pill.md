...
| `.-render-icon-<name>` | <span class="instui-pill -color-danger pantoken-doc-tag">더 이상 사용되지 않음</span> — The former `renderIcon` prop; still works as an alias, but use `-icon-<name>` instead. |
...
- [badge](/ko/api/css/badge.md) — A badge is the count or notification counterpart.
- [tag](/ko/api/css/tag.md) — A tag is the removable, form-oriented counterpart.
