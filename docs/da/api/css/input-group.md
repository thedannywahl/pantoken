...
`.instui-input-group` — En facade omkring et tekstinput med ledende og efterfølgende ikonslots.

Omslutter samme feltchrome som `text-input` med ikonslots på begge sider; `-should-not-wrap` er kun nødvendig, når gruppens indhold ellers ville løbe over til en anden linje.

**Kilde:** [input-group.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/input-group/input-group.css)
...
| `.-should-not-wrap` | Bevar gruppen på en linje (ingen ombrydning). |
...
| Del | Beskrivelse |
| --- | --- |
| `.after` | Indholdsslotet bagud. |
| `.before` | Indholdsslotet forude. |
...
- [text-input](/da/api/css/text-input.md) — Omslutter et tekstinput og tilføjer ledende og efterfølgende ikonslots.
