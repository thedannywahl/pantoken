...
`tr` — En tabelrække (InstUI `Table.Row`).

Forælderen `table`'s `-hover` og `-layout-stacked` modifiers restyler dette medlem — se `table`'s egen dokumentation for disse modifiers.
