...
`.--text-danger` — Semantiske farveværktøjer: `.--bg-&lt;name&gt;`, `.--text-&lt;name&gt;` (aliaseret som `.--color-&lt;name&gt;`) og `.--border-&lt;name&gt;` til det kurerede semantiske farvepalet. Hver af disse har også en komponent-tilknyttet aliasmodifier (f.eks. `-bg-danger` på enhver `.instui-&lt;component&gt;`).

**Kilde:** [generate.ts](https://github.com/thedannywahl/pantoken/blob/main/formats/components/scripts/generate.ts)
...
| --- | --- |
| `.--text-danger` | Anvender den semantiske faretekstfarve. |
| `.--bg-*` | Baggrundsfarveværktøjer til semantiske og komponentjusterede navne. |
| `.--border-*` | Kantfarveværktøjer til semantiske og komponentjusterede navne. |
| `.--color-*` | Alias på `--text-*` for tekstfarveværktøjer. |
| `.--text-*` | Tekstfarveværktøjer til semantiske og komponentjusterede navne. |
