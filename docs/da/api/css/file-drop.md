...
`.instui-file-drop` — En fildropzone med svæve-, accepteret og afviste tilstande.

`-hover`/`-accepted`/`-rejected` klasserne er rent visuelt — faktisk træk-og-slip-detektion og filvalidering er forbrugerens JS for at forbinde omkring det omsluttede native input.

**Kilde:** [file-drop.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/file-drop/file-drop.css)

## Accessibility

Ombyd et naturligt `<input type="file">` i `&lt;label&gt;` dropzone, så det forbliver et rigtigt, mærket filkontrol, som tastaturet og hjælpeteknologi kan betjene.
...
| ------------ | ------------------------------------- |
| `.-accepted` | Træktilstand for en acceptabel fil.   |
| `.-hover`    | Svæve- eller træk-over-tilstand.      |
| `.-icon-*`   | Gengiv det førende dropzone-glyfikon. |
| `.-rejected` | Træktilstand for en afvist fil.       |
