...
`:where(body)` — Typografiske standarder for rå HTML — overskrifter, afsnit, lister, links og kode — anvendt automatisk overalt hvor det importeres (standard `:where(body)`); pass `options.scope` for at målrette en anden indholds rod i stedet (f.eks. `.vp-doc`).

**Kilde:** [index.ts](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/rules/prose/index.ts)
