...
`.day` — En dag-celle (InstUI `Calendar.Day`); `-today`, `-selected` og `-outside-month` markerer dens tilstand.
...
| ----------------- | ------------------------------------------------------------------ |
| `.-outside-month` | En dag fra den tilstødende måned (ledende/efterfølgende fyldstof). |
| `.-selected`      | Den valgte dato.                                                   |
| `.-today`         | Dagens dato.                                                       |
