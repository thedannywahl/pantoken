...
`.instui-toggle-details` — En stiliseret native `&lt;details&gt;` disclosure med en roterende chevron.

Bygget på det native `&lt;details&gt;` element, så browseren driver åbning og lukning plus tastaturunderstøttelse; CSS'en skjuler kun standardmarkøren og leverer den roterende chevron.

**Kilde:** [toggle-details.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/toggle-details/toggle-details.css)
...
| --- | --- |
| `.-chevron-end` | Placer chevron efter sammenfattelsen. |
| `.-size-large` | Stor. Langt-form alias af `-size-lg`. |
| `.-size-lg` | Stor. |
| `.-size-sm` | Lille. |
| `.-size-small` | Lille. Langt-form alias af `-size-sm`. |
| `.-variant-filled` | Fyldt (overflade) variant. |

## Pseudo-elementer

| Pseudo-element | Beskrivelse |
| --- | --- |
| `::before` | Tegner sammenfattelsens disclosure chevron, en maskeret glyf, der roterer for at pege ned, når disclosure er åbent. |
...
- [toggle-group](/da/api/css/toggle-group.md) — Den afgrænsede, grupperede form af samme åbenbaring.
