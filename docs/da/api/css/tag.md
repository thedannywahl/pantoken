...
`.instui-tag` — Et inline chip for et nøgleord eller filter.

Indstiller sin egen horisontale `padding` (`-size-sm`/`-size-lg` skalerer det); kombination med en `-p-*`/`-padding-*` spacing-utility-modifier overskriver denne indbyggede værdi.

**Kilde:** [tag.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/tag/tag.css)
...
| --- | --- |
| `.-inline` | Læses inline med tekst og får en dismiss-glyf bagved. |
| `.-readonly` | Skrivebeskyttet (ikke-afvisbar) tag. |
| `.-size-large` | Et stort tag. Lang-form alias af `-size-lg`. |
| `.-size-lg` | Et stort tag. |
| `.-size-sm` | Et lille tag. |
| `.-size-small` | Et lille tag. Lang-form alias af `-size-sm`. |

## Pseudo-elementer

| Pseudo-element | Beskrivelse |
| --- | --- |
| `::after` | Renderer dismiss-glyfen bagved på et afvisbar inline tag. |
...
- [pill](/da/api/css/pill.md) — Den skrivebeskyttede label-chip modpart.
