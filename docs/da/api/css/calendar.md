...
`.instui-calendar` — Et statisk månedgitter med navigation, ugedags-headers og dagceller.

Se `calendar.day` medlemmet for de individuelle dagceller.

**Kilde:** [calendar.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/calendar/calendar.css)

## Tilgængelighed

Udstil gitteret med `role="table"` og en beskrivende `aria-label`, og giv hver navigationknap sin egen `aria-label`.
...
| Del | Beskrivelse |
| --- | --- |
| `.grid` | Det syv-kolonne dagsgitter. |
| `.nav` | Månednavigations-rækken. |
| `.weekday` | En ugedags-kolonne header. |
...
- [calendar.day](/da/api/css/calendar.day.md)
