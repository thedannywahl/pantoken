...
- f401dfb: Fix the localized API docs build: bare-HTML escaping now matches code spans by backtick-run length, like CommonMark, so a translated string with a stray double backtick can't leave a raw `<name>` tag that breaks the Vue compile.
...
- 6888f57: Fix the "Deploy to Netlify" step failing with `Error: Projects detected: ...` and refusing to deploy. Netlify CLI 16+ scans for a pnpm workspace and, run from the repo root, won't guess which of this monorepo's ~100 packages to deploy without a `--filter`, even with `NETLIFY_SITE_ID` set. The deploy command now passes `--filter "@pantoken/docs"`.
...
- da1ee24: Move the docs site from GitHub Pages to Netlify, and cut the deploy build's peak memory.
...
- 1d93845: fix(docs): scope `vitepress-plugin-llms` to the English locale and one build pass, and raise the docs deploy heap ceiling
...
- 28e42c9: fix(docs): declare `@pantoken/pendo` as a devDependency
