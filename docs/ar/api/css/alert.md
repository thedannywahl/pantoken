...
`.instui-alert` · <span class="instui-pill -color-success pantoken-doc-tag">stable</span> — رسالة داخلية قصيرة مع شريط حالة ملون وصورة حالة مخفية من مجموعة الأيقونات المشتركة.

يستبدل `-icon-&lt;name&gt;` المخصص صورة الحالة لكنه يحتفظ بشريط اللون الخاص بالنسخة؛ يتم إعادة تطبيق تعبئة الشريط بخصوصية أعلى حتى لا يلتهمها رسام الأيقونات المشترك. زر الإغلاق المباشر يكون بحجم صغير افتراضيًا؛ أضف معدل حجم لتجاوزه.

**المصدر:** [alert.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/alert/alert.css)

<!-- js-requirement -->
> [!TIP]
> **تحسين JS** — تعمل وتُعرض أنماط CSS الخاصة بهذا المكوّن بمفردها؛ ازجه مع `@pantoken/interactions` لإضافة السلوك التفاعلي. معدل `-timeout` الخاص به يُدار بواسطة ذلك السلوك. انظر [جدول المعدلات أدناه](#modifiers).


## سهولة الوصول

لرسالة مهمة، أضف `role="alert"` أو منطقة `aria-live` حتى تعلنها تقنيات المساعدة؛ عنصر الإغلاق قابل للإلغاء هو زر إغلاق معنًى (الـ `.instui-close-button` في المثال يحمل `aria-label="Close"`).
...
| فتحة | الوصف |
| --- | --- |
| `content` | محتوى رسالة التنبيه؛ قد يتضمن زر إغلاق. |
...
| `.-color-info` | معلومة (افتراضي). |
| `.-color-success` | رسالة إيجابية/تأكيد. |
...
| `.-icon-*` | استبدل صورة الحالة بأيقونة مخصصة (مثلاً `-icon-megaphone`), تُحافظ باللون الأبيض فوق شريط النسخة الملون. |
| `.-render-custom-icon-*` | <span class="instui-pill -color-danger pantoken-doc-tag">مهجور</span> — The former `renderCustomIcon` prop; still works as an alias, but use `-icon-<name>` (or override `--pantoken-alert-glyph`) instead. |
| `.-screen-reader-only` | مخفي بصريًا لكن يتم الإعلان عنه. |
...
| `.-timeout-3` | ثلاث ثوانٍ قبل الإغلاق التلقائي. |
| `.-timeout-4` | أربع ثوانٍ قبل الإغلاق التلقائي. |
| `.-timeout-5` | خمس ثوانٍ قبل الإغلاق التلقائي. |
| `.-variant-error` | <span class="instui-pill -color-danger pantoken-doc-tag">مهجور</span> — use `.-color-danger`. |
| `.-variant-info` | <span class="instui-pill -color-danger pantoken-doc-tag">مهجور</span> — use `.-color-info`. |
| `.-variant-success` | <span class="instui-pill -color-danger pantoken-doc-tag">مهجور</span> — use `.-color-success`. |
| `.-variant-warning` | <span class="instui-pill -color-danger pantoken-doc-tag">مهجور</span> — use `.-color-warning`. |
...
| عنصر زائف | الوصف |
| --- | --- |
| `::after` | صورة الحالة البيضاء، مُقنعة ومتمركزة فوق الشريط. |
| `::before` | شريط الحالة الصلب الملون للنسخة، ملتصق بحافة البداية المستديرة. |

## خصائص مخصّصة

| خاصية | نوع | افتراضي | الوصف |
| --- | --- | --- | --- |
| `--pantoken-alert-glyph` | `<url>` | — | مصدر صورة الحالة منخفض المستوى؛ `-icon-&lt;name&gt;` يعيّنه لك. استبدله لأيقونة مخصصة (SVG مشفر في عنوان URL). |
| `--pantoken-alert-icon-bg` | `<color>` | — | تعبئة شريط الحالة الملون خلف الصورة؛ كل نسخة `-color-*` تحدد تعبئتها الخاصة. |
| `--timeout` | `<integer>` | — | ملّي ثانية قبل الإغلاق التلقائي؛ `0` يعطّلها. يتطلب حزمة تفاعل التنبيه. |
...
- [close-button](/ar/api/css/close-button.md) — عنصر الإغلاق الذي قد يتضمنه التنبيه.
