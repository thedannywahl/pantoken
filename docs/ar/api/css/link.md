...
`.instui-link` — ارتباط تشعبي مصمم مع أحجام وتباين معكوس للخلفيات الداكنة وأشكال مضمنة أو بدون تنسيق.

يعين `gap` سريع الاستجابة الخاص به بين أيقونة ونصه (`-size-sm`/`-size-lg` تعديله)؛ ربط معدِّل أداة `-gap-*` للمسافة يلغي تلك القيمة المدمجة.

**المصدر:** [link.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/link/link.css)

## Accessibility

ضع علامة على رابط معطَّل باستخدام `aria-disabled="true"`.
...
| `.-inline`        | رابط مضمَّن، بخط تحتي في النص الجاري.        |
| `.-lg`            | رابط مضمَّن كبير (مستخدم مع `-inline`).      |
| `.-size-large`    | كبير. اسم مستعار طويل الشكل لـ `-size-lg`.   |
| `.-size-lg`       | كبير.                                        |
| `.-size-sm`       | صغير.                                        |
| `.-size-small`    | صغير. اسم مستعار طويل الشكل لـ `-size-sm`.   |
| `.-sm`            | رابط مضمَّن صغير (مستخدم مع `-inline`).      |
| `.-unstyled`      | إزالة نمط الرابط: وراثة اللون، بدون خط تحتي. |
...
- [breadcrumb](/ar/api/css/breadcrumb.md) — مسار breadcrumb مبني من روابط؛ أيضاً بديله للشاشة الصغيرة (اقرن مع أداة `responsive` لتبديل Breadcrumb برابط أقل من ~768px).
