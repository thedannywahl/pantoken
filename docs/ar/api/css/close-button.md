...
<!-- js-requirement -->
> [!TIP]
> **تحسين JS** — تنسيقات CSS لهذا المكوّن تعرض وتعمل بمفردها؛ زوجها مع `@pantoken/interactions` لإضافة السلوك التفاعلي. راجع [جدول المعدِّلات أدناه](#modifiers).
...
| `.-size-large` | كبير. اسم مُطوّل لـ `-size-lg`. |
| `.-size-lg` | كبير. |
| `.-size-sm` | صغير. |
| `.-size-small` | صغير. اسم مُطوّل لـ `-size-sm`. |
