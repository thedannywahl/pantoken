...
`:where(body)` — الافتراضيات الطباعية لـ HTML الخام — العناوين والفقرات والقوائم والروابط والكود — تطبق تلقائياً حيثما يتم استيرادها (افتراضي `:where(body)`؛ مرر `options.scope` لاستهداف جذر محتوى مختلف بدلاً من ذلك (على سبيل المثال `.vp-doc`).
