...
`:where(body)` — الإعدادات الطباعية الافتراضية لـ HTML الخام — العناوين، الفقرات، القوائم، الروابط، والكود — تُطبق تلقائيًا في أي مكان يتم فيه استيراده (الافتراضي `:where(body)`); مرّر `options.scope` لاستهداف جذر محتوى مختلف بدلاً من ذلك (مثال `.vp-doc`).

**المصدر:** [index.ts](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/rules/prose/index.ts)
