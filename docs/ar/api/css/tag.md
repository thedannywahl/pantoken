...
`.instui-tag` — رقاقة مضمّنة لكلمة مفتاحية أو مرشح.

تحدد `padding` الأفقية الخاصة بها (`-size-sm`/`-size-lg` يقومان بتغييرها)؛ سيسيطِر ربط معدل مسافة أداة المساعدة `-p-*`/`-padding-*` على تلك القيمة المضمّنة.

**المصدر:** [tag.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/tag/tag.css)
...
| معدّل | الوصف |
| --- | --- |
| `.-inline` | يُقرأ في سطر النص ويحصل على رمز إغلاق لاحق. |
| `.-readonly` | وسم للقراءة فقط (لا يمكن إغلاقه). |
| `.-size-large` | وسم كبير. اسم بديل طويل الشكل لـ `-size-lg`. |
| `.-size-lg` | وسم كبير. |
| `.-size-sm` | وسم صغير. |
| `.-size-small` | وسم صغير. اسم بديل طويل الشكل لـ `-size-sm`. |

## عناصر زائفة

| عنصر زائف | الوصف |
| --- | --- |
| `::after` | يعرض رمز الإغلاق اللاحق على الوسم المضمّن القابل للإغلاق. |
...
- [pill](/ar/api/css/pill.md) — النظير الخاص بالوسم-رقاقة للقراءة فقط.
