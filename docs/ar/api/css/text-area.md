...
`.instui-text-area` — عنصر `&lt;textarea&gt;` أصلي ذو نمط قابل لتغيير الحجم بنفس حالات الحجم والحالات الوظيفية لحقل النص.

يتشارك الحدّ والخلفية وحالات التركيز مع `text-input` و`simple-select`؛ ما يميّزه فقط هو `resize: vertical` وارتفاع الحد الأدنى الأكبر.

**المصدر:** [text-area.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/text-area/text-area.css)
...
| `.-invalid` | الحالة غير صالحة (خطأ). |
| `.-readonly` | حالة للقراءة فقط. |
| `.-size-large` | كبير. اسم طويل لـ `-size-lg`. |
| `.-size-lg` | كبير. |
| `.-size-md` | (الافتراضي) متوسط. |
| `.-size-medium` | (الافتراضي) متوسط. اسم طويل لـ `-size-md`. |
| `.-size-sm` | صغير. |
| `.-size-small` | صغير. اسم طويل لـ `-size-sm`. |
| `.-success` | نجاح (صالح) الحالة. |

## عناصر زائفة

| عنصر زائف | الوصف |
| --- | --- |
| `::placeholder` | نص العنصر النائب، بلون باهت يتغير عند التمرير فوقه. |
...
- [text-input](/ar/api/css/text-input.md) — النظير ذو السطر الواحد بنفس الحالات والأحجام.
