...
`.instui-number-input` — واجهة إدخال رقم مع عمود دوار +/-.

أزرار `.arrows` هي `aria-hidden` وذات طابع عرضي فقط — زيادة/نقصان القيمة عند النقر هي مسؤولية جافاسكربت الخاص بالمستهلك لربطها.

**المصدر:** [number-input.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/number-input/number-input.css)

## سهولة الوصول

ضع الاسم الوصولي على `&lt;input&gt;` (مثلاً `aria-label`) بحيث يمتلك الحقل القيمة؛ أزرار الدوار +/- زخرفية ومعلّمة كـ `aria-hidden="true"`.
...
| معدّل | الوصف |
| --- | --- |
| `.-disabled` | حالة معطّلة. |
| `.-invalid` | حالة غير صالحة (خطأ). |
| `.-readonly` | حالة للقراءة فقط. |
| `.-size-large` | كبير. اسم بديل مطوّل لـ `-size-lg`. |
| `.-size-lg` | كبير. |
| `.-size-md` | (الافتراضي) متوسط. |
| `.-size-medium` | (الافتراضي) متوسط. اسم بديل مطوّل لـ `-size-md`. |
| `.-size-sm` | صغير. |
| `.-size-small` | صغير. اسم بديل مطوّل لـ `-size-sm`. |
| `.-success` | حالة نجاح (صالح). |

## الأجزاء

| جزء | الوصف |
| --- | --- |
| `.arrows` | عمود الدوار +/- في نهاية السطر الداخلي. |
| `.down` | زر النقصان؛ الزر غير المصنّف يزيد القيمة. |

## عناصر زائفة

| عنصر زائف | الوصف |
| --- | --- |
| `::before` | رمز الشيفرون المقنّع على كل زر دوار: أعلى للزيادة، وأسفل على `.down`. |
...
- [text-input](/ar/api/css/text-input.md) — يشارك نفس واجهة المدخل.
- [range-input](/ar/api/css/range-input.md) — عنصر تحكم رقمي آخر.
