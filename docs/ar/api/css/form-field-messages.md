...
`.instui-form-field-messages` — رسائل المساعدة والتحقق من الحقل — تلميح وخطأ ونجاح وقارئ شاشة فقط — مع حرف خاص في حالة الخطأ والنجاح.

تعيين `gap` الخاص به بين الرسائل المكدسة؛ ربط معدل أداة تباعد `-gap-*` يتجاوز تلك القيمة المدمجة.
...
رسالة `-type-screenreader-only` مخفية بصرياً لكنها تبقى في شجرة الوصول، لذلك يتم الإعلان عنها بعد؛ اربط رسائل الخطأ والنجاح بالحقل عبر aria-describedby بحيث تقرأ تقنيات المساعدة معها المراقب.
...
| Part                         | Description                                                                                  |
| ---------------------------- | -------------------------------------------------------------------------------------------- |
| `.instui-form-field-message` | رسالة فردية؛ `-type-*` الخاصة بها تختار متغير التلميح أو الخطأ أو النجاح أو قارئ الشاشة فقط. |

## Pseudo-elements

| Pseudo-element | Description                                                                                       |
| -------------- | ------------------------------------------------------------------------------------------------- |
| `::before`     | يرسم الحرف الخاص الحالة الرائدة في رسائل الخطأ والنجاح: دائرة تنبيه للأخطاء ودائرة اختيار للنجاح. |
...
- [form-field](/ar/api/css/form-field.md) — يلف تسمية والمراقبات وهذه الرسائل.
