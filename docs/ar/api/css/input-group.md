...
| معدّل | الوصف |
| --- | --- |
| `.-disabled` | الحالة المعطلة. |
| `.-invalid` | الحالة غير الصالحة (خطأ). |
| `.-readonly` | حالة القراءة فقط. |
| `.-should-not-wrap` | إبقاء المجموعة في سطر واحد (بدون التفاف). |
| `.-size-large` | كبير. تسمية طويلة لـ `-size-lg`. |
| `.-size-lg` | كبير. |
| `.-size-md` | متوسط. |
| `.-size-medium` | متوسط. تسمية طويلة لـ `-size-md`. |
| `.-size-sm` | صغير. |
| `.-size-small` | صغير. تسمية طويلة لـ `-size-sm`. |
| `.-success` | حالة النجاح (صالِح). |
...
| عنصر زائف | الوصف |
| --- | --- |
| `::placeholder` | نص العنصر النائب للحقل الداخلي، بلون خافت. |
