...
`@pantoken/canvas-theme-editor` — `theme.css`/`theme.js` جاهزة للتحميل لمحرر Theme في Canvas LMS، مربوطة مسبقًا بـ CSS المستضافة على CDN من pantoken. يتم استهلاكها بواسطة منصة `@pantoken/scaffold`' `canvas-theme-editor`؛ يمكن أيضًا استخدامها بشكل مستقل.

`THEME_CSS`/`THEME_JS` هي الإخراج الافتراضي jsDelivr غير المثبت — استخدم [buildTheme](functions/buildTheme.md) للاستهداف موفر CDN مختلف، أو تثبيت إصدار، أو استبدال أصول pantoken/جهات خارجية أخرى.
