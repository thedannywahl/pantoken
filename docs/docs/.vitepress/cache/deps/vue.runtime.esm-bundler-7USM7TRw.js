...
function isNodeMismatchAllowed(node, vnode) {
  return (
    isMismatchAllowed(node.parentElement, 1) ||
...
function renderSlot(slots, name, props, fallback, noSlotted, branchKey) {
  ...
  if (
    ...
    const slotProps =
      branchKey != null && props.key == null ? extend({}, props, { key: branchKey }) : props;
    ...
    return {
      ...
      set(value) {
        ...
        if (
          ...
          ((hasChanged(value, emittedValue) && !hasChanged(emittedValue, prevEmittedValue)) ||
...
function createBaseVNode(
  ...
  if (props && vnode.shapeFlag & 1) {
    const overwritingProp =
      props.innerHTML != null ? "innerHTML" : props.textContent != null ? "textContent" : null;
