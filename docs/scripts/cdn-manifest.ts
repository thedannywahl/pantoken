 ...
 * (the `kind: "component"` records that get a per-component `<name>.css` on the CDN), whether each
 * references icons (so the picker knows to add `component-icons.css` to the combine URL), and the
 * current published versions of `@pantoken/components` + `@pantoken/css` (so the generated URLs pin to
 * a real version). Sourced from the component registry — never hand-maintained.
 *
...
const versionOf = (pkg: string): string =>
  (JSON.parse(readFileSync(resolve(root, pkg, "package.json"), "utf8")) as { version: string })
    .version;
...
const manifest = {
  componentsVersion: versionOf("formats/components"),
  cssVersion: versionOf("formats/css"),
  components,
};
