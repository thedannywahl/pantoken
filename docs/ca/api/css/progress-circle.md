...
> [!TIP]
> **JS Enhancement** — El CSS d'aquest component es renderitza i funciona per si sol; parella'l amb `@pantoken/interactions` per afegir el comportament interactiu. El seu modificador `-should-animate` està impulsat per aquest comportament. Veu la [taula de modificadors més avall](#modifiers).
