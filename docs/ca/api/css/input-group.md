...
`.instui-input-group` — Una façana al voltant d'una entrada de text amb ranures d'icona inicial i final.

Embolica el mateix chrome del camp que `text-input` amb ranures d'icona a ambdós costats; `-should-not-wrap` només és necessari quan el contingut del grup desbordarà d'una altra manera a una segona línia.

**Font:** [input-group.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/input-group/input-group.css)
...
| `.-should-not-wrap` | Mantingueu el grup en una línia (sense embolicament). |
...
| Part      | Description                     |
| --------- | ------------------------------- |
| `.after`  | La ranura de contingut final.   |
| `.before` | La ranura de contingut inicial. |
...
- [text-input](/ca/api/css/text-input.md) — Embolica una entrada de text, afegint ranures d'icona inicial i final.
