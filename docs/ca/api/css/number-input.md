...
`.instui-number-input` — Una façana d'entrada de número amb una columna de filador +/-.

Els botons `.arrows` són `aria-hidden` i purament de presentació — incrementar/decrementar el valor al fer clic és el JS del consumidor per connectar.

**Font:** [number-input.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/number-input/number-input.css)

## Accessibility

Poseu el nom accessible a `&lt;input&gt;` (p. ex. `aria-label`) perquè el camp sigui propietari del valor; els botons de filador +/- són decoratius i marcats `aria-hidden="true"`.
...
| Part      | Description                                            |
| --------- | ------------------------------------------------------ |
| `.arrows` | La columna de filador +/- al final en línia.           |
| `.down`   | El botó de decrement; el botó sense classe incrementa. |

## Pseudo-elements

| Pseudo-element  | Description                                                                                     |
| --------------- | ----------------------------------------------------------------------------------------------- |
| `::before`      | El glifo de chevron emmascarrat a cada botó de filador: amunt per incrementar, avall a `.down`. |
| `::placeholder` | El text de marcador de posició de l'entrada interna, en un color silenciat.                     |
...
- [text-input](/ca/api/css/text-input.md) — Comparteix la mateixa façana de chrome d'entrada.
- [range-input](/ca/api/css/range-input.md) — Un altre control d'entrada numèrica.
