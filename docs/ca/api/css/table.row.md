...
`tr` — Una fila de taula (InstUI `Table.Row`).

Els modificadors `-hover` i `-layout-stacked` del pare `table` reformaten aquest membre — consulta la documentació pròpia de `table` per a aquests modificadors.
