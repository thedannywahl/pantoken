...
`.--text-danger` — Utilitats de color semàntic: `.--bg-&lt;name&gt;`, `.--text-&lt;name&gt;` (amb àlies com `.--color-&lt;name&gt;`), i `.--border-&lt;name&gt;` per a la paleta semàntica revisada. Cadascun d'aquests també té un modificador d'àlies adjunt al component (per exemple `-bg-danger` en qualsevol `.instui-&lt;component&gt;`).

**Font:** [generate.ts](https://github.com/thedannywahl/pantoken/blob/main/formats/components/scripts/generate.ts)
...
| ---------------- | --------------------------------------------------------------------------- |
| `.--text-danger` | Aplica el color de text de perill semàntic.                                 |
| `.--bg-*`        | Utilitats de color de fons per a noms semàntics i alineats amb components.  |
| `.--border-*`    | Utilitats de color de límit per a noms semàntics i alineats amb components. |
| `.--color-*`     | Àlies de `--text-*` per a utilitats de color de text.                       |
| `.--text-*`      | Utilitats de color de text per a noms semàntics i alineats amb components.  |
