...
`.instui-tag` — Una fitxa en línia per a una paraula clau o filtre.

Estableix el seu propi `padding` horitzontal (`-size-sm`/`-size-lg` l'escalen); encadenar un modificador d'utilitat d'espaiat `-p-*`/`-padding-*` substitueix aquest valor integrat.

**Font:** [tag.css](https://github.com/thedannywahl/pantoken/blob/main/formats/components/src/components/tag/tag.css)
...
| --- | --- |
| `.-inline` | Es llegeix en línia amb el text i obté un glifo de descarta final. |
| `.-readonly` | Etiqueta de només lectura (no descartable). |
| `.-size-large` | Una etiqueta gran. Àlies de forma llarga de `-size-lg`. |
| `.-size-lg` | Una etiqueta gran. |
| `.-size-sm` | Una etiqueta petita. |
| `.-size-small` | Una etiqueta petita. Àlies de forma llarga de `-size-sm`. |

## Pseudoelements

| Pseudoelement | Descripció |
| --- | --- |
| `::after` | Representa el glifo de descarta final en una etiqueta en línia descartable. |
...
- [pill](/ca/api/css/pill.md) — L'homòleg de l'etiqueta de només lectura.
