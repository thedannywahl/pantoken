...
test("every registered definition is a single well-formed record (sanity: the registries are populated)", () => {
  // COMPONENTS is the load-bearing concat order (71: 49 components + icon/mask/screen-reader-content
  // + the `menu.item`/`menu.group`/`menu.separator`, `tabs.tab`/`tabs.panel`, `breadcrumb.link`,
  // `calendar.day`, `pagination.page`, `side-nav-bar.item`, `modal.header`/`modal.body`/`modal.footer`,
...
test("every registered definition has a unique name (guards @memberOf cross-file resolution)", () => {
  // `icon`/`mask`/`screen-reader-content` intentionally appear in both COMPONENTS (their
  // `componentsCss()` concat position) and UTILITIES (the same `Definition` object, for `validate()`
  // coverage) — dedupe by object identity first so that legitimate double-registration isn't mistaken
  // for a name collision between two DIFFERENT records.
