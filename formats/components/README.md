...
| `.instui-byline`              | `.hero` (figure), `.title`, `.description`; `-align-content-top`, `-size-{sm,md,lg}` (max-width)                                                                                                                           |
| `.instui-img`                 | styled `<img>`; `-display-block`, `-constrain-{cover,contain}`, `-with-grayscale`, `-with-blur` (effects stack)                                                                                                            |
| `.instui-table`               | styles `th`/`td` + `caption`; `-hover` (row highlight), `-layout-fixed`, `-layout-stacked` (card rows via per-cell `data-label`)                                                                                           |
| `.instui-link`                | hover, `[aria-disabled]`, `-size-*`, `-color-inverse` (on dark), `-inline`, `-unstyled`; icons via an `.instui-icon` child (placement = DOM order)                                                                         |
...
| `.instui-spinner`             | animated CSS ring; `-size-{xs,sm,lg}`, `-color-inverse` (on dark)                                                                                                                                                          |
| `.instui-progress`            | `.bar` meter; `-color-{brand,info,success,warning,danger}` (root; deprecated `-meter-color-*`), `-color-primary-inverse` (on-dark), `-size-*`, `-should-animate`, `.instui-progress-value` label                           |
| `.instui-menu`                | `.item`, `.separator`                                                                                                                                                                                                      |
| `.instui-modal`               | `.header`, `.body`, `.footer`; sizes, `-density-compact`, `-color-inverse`; a `.body` with an `<img>` goes full-bleed (media modal)                                                                                        |
| `.instui-breadcrumb`          | `.item` (with `/` separators; last is the current page), `-size-*`; crumbs can lead with an `.instui-icon`                                                                                                                 |
| `.instui-billboard`           | `.hero` (icon/image), `.heading`, `.message`; `-size-*`, `-clickable`                                                                                                                                                      |
| `.instui-rating`              | star glyphs (`.instui-icon.-icon-star-solid` filled / `.-icon-star` empty), `.label` (e.g. "3/5"), `-size-*`                                                                                                               |
| `.instui-toggle-group`        | a bordered `<details>` disclosure (chevron summary row + collapsible content); `-size-*`, `-without-border`                                                                                                                |
| `.instui-context-view`        | an elevated callout with a caret; `-placement-{top,bottom,start,end}`, `-color-inverse`; works as a native `[popover]`                                                                                                     |
| `.instui-progress-circle`     | a ring driven by a `--value` (0–100) custom property; `-color-{brand,info,success,warning,danger}` (deprecated `-meter-color-*`), `-color-primary-inverse`, `-size-*`                                                      |
| `.instui-pagination`          | `.page` (`<a>`/`<button>`; current = `[aria-current]`/`-current`), `.arrow` (first/prev/next/last), `.ellipsis`, `-variant-input`                                                                                          |
...
| `.instui-side-nav-bar`        | vertical rail of icon-over-label `.item`s; `-selected`, `-minimized` (icons only)                                                                                                                                          |
| `.instui-tree-browser`        | disclosure tree of nested `<details>` collections + `.item` leaves (rotating chevron); `-size-{sm,lg}`                                                                                                                     |
| `.instui-calendar`            | static month grid: `.nav`, `.grid`, `.weekday`, `.day` (`-today`/`-selected`/`-outside-month`)                                                                                                                             |
| `.instui-popover`             | elevated surface for a native `[popover]`; `-placement-*` + CSS anchor positioning (anchor the trigger with `anchor-name: --pantoken-anchor`) and a `@starting-style` open animation, both `@supports`-gated               |
| `.instui-tray`                | edge-pinned panel: `-placement-{start,end,top,bottom}`, `-size-{xs,sm,lg,xl}`; native `[popover]`/`<dialog>`                                                                                                               |
| `.instui-tooltip`             | CSS hover/focus bubble (`.tip`); `-placement-*` (web-component adds delay/dismiss/`aria-describedby`)                                                                                                                      |
...
| `.instui-text-input`          | styled native `<input>` (incl. `type="date"`/`time`/`datetime-local` — the browser supplies the picker); `-invalid`, `-success`, `-readonly`, `-size-{sm,md,lg}`                                                           |
