...
import { generateLocaleBundles, parseI18nSource } from "@pantoken/translation-adapters";
...
for (const platform of readdirSync(templatesRoot, { withFileTypes: true })) {
  ...
  if (existsSync(templateI18nPath)) {
    const { strings } = parseI18nSource(JSON.parse(readFileSync(templateI18nPath, "utf8")));
