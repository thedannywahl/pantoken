...
- Browser interaction configuration, including component attributes and CSS custom properties such as Alert's `--timeout`, is application-controlled. Interaction helpers validate expected numeric ranges, but applications remain responsible for deciding which elements may be dismissed.
