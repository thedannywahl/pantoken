...
- Browser component configuration, including numeric attributes and CSS custom properties such as ProgressBar's `value-now`/`value-max`, Alert's `--timeout`, and ProgressCircle's `--animation-delay`, is application-controlled. Web components and interaction helpers validate expected numeric ranges, but applications remain responsible for deciding which elements may be updated, dismissed, or animated.
