...
Each release includes two attached assets: the package tarball (`.tgz`) and a SLSA provenance bundle (`*.intoto.jsonl`) generated by `actions/attest-build-provenance`. The provenance bundle is a signed statement linking the tarball to the exact source commit and GitHub Actions workflow that produced it.
